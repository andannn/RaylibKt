...
class MultiTargetNativeCompilation(
    ...
    fun sharedObjectOutputFor(konanTarget: KonanTarget): Provider<RegularFile> =
        nativeTargets.named(konanTarget.name).flatMap { nativeTargetCompilation ->
    ...
    private class Factory(
        ...
        override fun create(name: String): NativeTargetCompilation = create(SerializableKonanTarget(name))

        @JvmName("createWithSerializableKonanTarget")
        private fun create(serializableKonanTarget: SerializableKonanTarget): NativeTargetCompilation {
        ...
        private fun createArchiveTask(
            ...
            val archiveTask =
                project.tasks.register(archiveTaskName, ClangArchiveTask::class.java) {
                    ...
                    llvmArchiveParameters.let { llvmAr ->
        ...
        private fun createCompileTask(
            ...
            val compileTask =
                project.tasks.register(compileTaskName, ClangCompileTask::class.java) {
                    usesService(KonanBuildService.obtain(project))
                    clangParameters.let { clang ->
                        clang.output.set(
        ...
        private fun createLinkerTask(
            ...
            val archiveTask =
                project.tasks.register(archiveTaskName, ClangLinkerTask::class.java) {
                    val konanTarget = serializableKonanTarget.asKonanTarget
