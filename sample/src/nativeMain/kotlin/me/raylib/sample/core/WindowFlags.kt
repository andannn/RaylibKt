    ...
    window(
        ...
        gameLoopEffect {
            onDraw {
                ...
                if (ConfigFlags.FLAG_FULLSCREEN_MODE.isEnabled()) DrawText("[F] FLAG_FULLSCREEN_MODE: on", 10, 80, 10, LIME)
                else DrawText("[F] FLAG_FULLSCREEN_MODE: off", 10, 80, 10, MAROON)

                if (ConfigFlags.FLAG_WINDOW_RESIZABLE.isEnabled()) DrawText("[R] FLAG_WINDOW_RESIZABLE: on", 10, 100, 10, LIME)
                else DrawText("[R] FLAG_WINDOW_RESIZABLE: off", 10, 100, 10, MAROON)

                if (ConfigFlags.FLAG_WINDOW_UNDECORATED.isEnabled()) DrawText("[D] FLAG_WINDOW_UNDECORATED: on", 10, 120, 10, LIME)
                else DrawText("[D] FLAG_WINDOW_UNDECORATED: off", 10, 120, 10, MAROON)

                if (ConfigFlags.FLAG_WINDOW_HIDDEN.isEnabled()) DrawText("[H] FLAG_WINDOW_HIDDEN: on", 10, 140, 10, LIME)
                else DrawText("[H] FLAG_WINDOW_HIDDEN: off (hides for 3 seconds)", 10, 140, 10, MAROON)

                if (ConfigFlags.FLAG_WINDOW_MINIMIZED.isEnabled()) DrawText("[N] FLAG_WINDOW_MINIMIZED: on", 10, 160, 10, LIME)
                else DrawText("[N] FLAG_WINDOW_MINIMIZED: off (restores after 3 seconds)", 10, 160, 10, MAROON)

                if (ConfigFlags.FLAG_WINDOW_MAXIMIZED.isEnabled()) DrawText("[M] FLAG_WINDOW_MAXIMIZED: on", 10, 180, 10, LIME)
                else DrawText("[M] FLAG_WINDOW_MAXIMIZED: off", 10, 180, 10, MAROON)

                if (ConfigFlags.FLAG_WINDOW_UNFOCUSED.isEnabled()) DrawText("[G] FLAG_WINDOW_UNFOCUSED: on", 10, 200, 10, LIME)
                else DrawText("[U] FLAG_WINDOW_UNFOCUSED: off", 10, 200, 10, MAROON)

                if (ConfigFlags.FLAG_WINDOW_TOPMOST.isEnabled()) DrawText("[T] FLAG_WINDOW_TOPMOST: on", 10, 220, 10, LIME)
                else DrawText("[T] FLAG_WINDOW_TOPMOST: off", 10, 220, 10, MAROON)

                if (ConfigFlags.FLAG_WINDOW_ALWAYS_RUN.isEnabled()) DrawText("[A] FLAG_WINDOW_ALWAYS_RUN: on", 10, 240, 10, LIME)
                ...
                if (ConfigFlags.FLAG_BORDERLESS_WINDOWED_MODE.isEnabled()) DrawText("[B] FLAG_BORDERLESS_WINDOWED_MODE: on", 10, 280, 10, LIME)
                ...
                if (ConfigFlags.FLAG_WINDOW_TRANSPARENT.isEnabled()) DrawText("FLAG_WINDOW_TRANSPARENT: on", 10, 360, 10, LIME)
